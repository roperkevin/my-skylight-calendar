...
* **The Tomorrow banner never shows up** — check `sensor.tomorrow_events` in Developer tools → States. If the sensor is *missing*, the package isn't loaded or HA wasn't restarted after adding it (see section 2). If it reads `0` even though tomorrow has events, look at its `queried_calendars` attribute — the calendar holding the event must be in that list (by default the sensor queries every calendar except weather feeds; if you replaced `wanted_calendars` with a manual list, entities that don't exist are skipped silently). Also note the sensor refreshes on a 15-minute cycle (and `unknown` right after a reload just means the first refresh hasn't run yet), so a freshly synced external event can take a few minutes to appear. In the dashboard *editor*, the card intentionally shows "visibility conditions not met" whenever the sensor is 0 — that's the conditional card doing its job, not an error.
