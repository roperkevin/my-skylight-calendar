...
* **The Tomorrow banner never shows up** — check `sensor.tomorrow_events` in Developer tools → States. If the sensor is *missing*, the package isn't loaded or HA wasn't restarted after adding it (see section 2). If it reads `0` even though tomorrow has events, the calendar holding them isn't in the sensor's `wanted_calendars` list, or its entity ID differs from the defaults — update the list (marked `UPDATE THESE ENTITIES` in the package file); calendars that don't exist in your install are skipped silently. Also note the sensor refreshes on a 15-minute cycle (and `unknown` right after a reload just means the first refresh hasn't run yet), so a freshly synced external event can take a few minutes to appear.
